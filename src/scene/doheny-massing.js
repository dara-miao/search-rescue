...
export function buildMassing(siteData, cfg = MASSING_CONFIG) {
  ...
  trimGeos.push(box(
    W + bc.projection * 2, bc.height, D + bc.projection * 2,
    0, bc.height / 2, 0
  ));
  ...
  trimGeos.push(box(
    W + sc.projection * 2, sc.height, D + sc.projection * 2,
    0, cfg.storeyHeight, 0
  ));
  ...
  brickGeos.push(box(
    pv.width, pavH, pv.projection,
    0, pavH / 2, D / 2 + pv.projection / 2
  ));
  trimGeos.push(box(
  ...
  // --- entrance portal: a deep recessed arch --------------------------
  // Depth is what sells this. A shallow arch reads as a sticker.
